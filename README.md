 ...
 * **The basic idea is that we will start with an object, like a sphere, then produce a chain of _domain distortions_ that will bend the sphere into a desired shape.**
...
* **For 2D, the image function is any picture or function we can render on the plane. For 3D, it is any implicit or volumetric function that can be rendered in a volume, like a sphere or mandlebulb fractal.**
...
* **This is a slightly different image function, where we have not restricted shading to the real line, but instead shade the whole negative half plane. If the image of a distorted position is negative, we shade it. The color or texture used is an artistic decision.**
...
* **If we want to restrict the distortion region, we can define an _isolation function  iso(x, y), 0 <= iso(x, y) <= 1_. This is used in the hyperplane-relative space.**
